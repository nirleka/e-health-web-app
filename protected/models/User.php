...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nomor_sip, password, nama, lokasi_penugasan, no_telp, role', 'required'),
			array('nomor_sip, password, nama, lokasi_penugasan', 'length', 'max'=>50),
			array('no_telp', 'length', 'max'=>20),
			array('role', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nomor_sip, password, nama, lokasi_penugasan, no_telp, role', 'safe', 'on'=>'search'),
		);
	}
